...
def build_retrieval_query(user_query: str, user_profile: Optional[dict]) -> str:
    ...
    parts.append("지원대상, 신청자격, 나이 조건, 사업자등록 요건, 업종 요건, 신청방법, 신청기간 중심으로 검색")
    return "\n".join(parts)
...
- 서비스의 목적, 대상 사용자, 주요 기능, 사용 방식을 중심으로 설명할 것
- 문서에 없으면 "문서상 확인되지 않음"이라고 답할 것
...
def build_multi_result_prompt(user_query: str, user_profile: Optional[dict], results: List[Dict]) -> str:
    ...
    return f"""
너는 한국의 정부지원정책, 창업지원, 정책자금, 대출 정보를 안내하는 친절한 한국어 챗봇이다.

반드시 아래 검색 결과 문서들만 바탕으로 답변하라.
문서에 없는 내용은 추측하지 말고, 사용자 추가 정보가 있으면 적합성 판단에 참고하라.
다만 문서에 없는 조건은 절대 단정하지 말고, 불확실하면 추가 확인 필요라고 설명하라.
